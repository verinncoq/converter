...
(*converts an initializer IR element into NNSequential
  output is an error_option of NNSequential element which can be
    1: NNSequential (in case of success, always with contructor NNSequential_initializer_vector or NNSequential_initializer_matrix)
    2: String (in case of error, containing the errormessage)
*)
Definition IR_initializer__to__NNSequential_IR__initializers (i: IR_elem) : error_option NNSequential :=
  match i with
  | IR_initializer s fv => match fv with
                           | value dim _ => match dim with
                                               ...
                                               | dim_vector n => match IR_fixedValue_vector_to_function fv with
                                                               | Some function => Success (NNSequential_initializer_vector s n function)
                                                               | None => Error ("Error in converting "++s++" to function.")
                                                               end
                                               | dim_matrix n m => match IR_fixedValue_matrix_to_function fv with
                                                               | Some function => Success (NNSequential_initializer_matrix s n m function)
                                                               | None => Error ("Error in converting "++s++" to function.")
                                                               end
                                               end
                           end
...
(*converts a node IR element into NNSequential
  output is an error_option of NNSequential element which can be
    1: NNSequential (in case of success, always with contructor NNSequential_Linear or NNSequential_ReLu)
    2: String (in case of error, containing the errormessage)
*)
Definition IR_node__to__NNSequential_IR (i: IR_elem) : error_option NNSequential :=
  match i with
  | IR_node s li lo op => match op with
                          | relu => (*in this model, relu must have exactly one input and one output*)
                                    match li, lo with
                                    | [input], [output] => Success (NNSequential_ReLu input output)
                          ...
                          | gemm alpha beta transA transB => (*in this model, relu must have exactly three inputs and one output*)
                                    ...
                                    | "1", "0" => match li, lo with
                                                      | input::weight::[bias], [output] => Success (NNSequential_Linear input output weight bias transB beta)
...
(*converts an output IR element into NNSequential
  output is an error_option of NNSequential element which can be
    1: NNSequential (in case of success, always with contructor NNSequential_Output)
    2: String (in case of error, containing the errormessage)
*)
Definition IR_output__to__NNSequential_IR (i: IR_elem) : error_option NNSequential :=
  match i with
  | IR_output s dim => match dim with
                       | dim_scalar => Error "The output dim cant be a scalar in this model."
                       | dim_vector n => Success (NNSequential_Output s n)
                       | dim_matrix n m => match n, m with
                                          | 1, _ => Success (NNSequential_Output s m)
                                          | _, 1 => Success (NNSequential_Output s n)
...
(*
convert_to_premodel module function. Performs all three functions defined above, applied on the IR_elems in the fourtuple.
Ignores input nodes, puts outputs in a list, when no error occurs. But if, first error is part of errormessage.
*)
Open Scope list_scope.
Definition convert_to_premodel (ft: fourtuple IR_elem) : error_option (list NNSequential) :=
  let inits := get_initializer ft in
  let nodes := get_nodes ft in
  let outputs := get_output ft in
  let error_option_inits := extract_error (map IR_initializer__to__NNSequential_IR__initializers inits) in
  let error_option_nodes := extract_error (map IR_node__to__NNSequential_IR nodes) in
  let error_option_outputs := extract_error (map IR_output__to__NNSequential_IR outputs) in
...
(*convert_to_premodel PROOF*)

Theorem same_node_count_convert_to_premodel: forall (ft: fourtuple IR_elem)(l: list NNSequential),
  convert_to_premodel ft = Success l -> length l = length (flatten_fourtuple_without_input ft).
Proof. intros. unfold flatten_fourtuple_without_input. unfold convert_to_premodel in H. destruct (extract_error
        (map IR_initializer__to__NNSequential_IR__initializers
           (get_initializer ft))) eqn:P1.
    - destruct (extract_error (map IR_node__to__NNSequential_IR (get_nodes ft))) eqn:P2.
       + destruct (extract_error (map IR_output__to__NNSequential_IR (get_output ft))) eqn:P3.
